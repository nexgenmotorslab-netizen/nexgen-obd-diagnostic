...
from scanner import connect_to_car, get_rpm, get_speed, get_coolant_temp, get_engine_load, get_throttle, clear_dtc_codes, get_dtc_codes,get_speed
...
def get_faults():
    ...
    codes = get_dtc_codes(car_conn)  # type: ignore
    
    if not codes:
        return {"status": "ok", "faults": "No fault codes found"}
    
    # If codes found, convert them to strings like "P0301"
    fault_list = [str(code) for code in codes]
    return {"status": "FAULTS FOUND", "faults": fault_list}
